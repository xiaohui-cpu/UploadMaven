package com.termend.service.impl.address;
...
import com.termend.mapper.AddressMapper;
import com.termend.pojo.Address;
import com.termend.pojo.AddressExample;
import com.termend.pojo.AddressExample.Criteria;
...
public class AddressServiceImpl implements IAddressService{
	...
	public void addAddress(Address address) {
		...
		AddressExample example = new AddressExample();
		Criteria criteria = example.createCriteria();
		criteria.andNameEqualTo(address.getName());
		criteria.andOpenidEqualTo(address.getOpenid());
		criteria.andGenderEqualTo(address.getGender());
		criteria.andAddressEqualTo(address.getAddress());
		criteria.andIsDefaultEqualTo(address.getIsDefault());
		
		Address addressNext=null;
		addressNext = addressMapper.selectByExample(example).get(0);
		if (address.getIsDefault() == 1) {
			// 如果是默认
			this.changeDefaultAddr(addressNext.getId(), addressNext.getOpenid());
		} else {
			// 如果不是默认
			// 看是否还有其他地址 没有也设为默认
			// 得到所有地址列表
